...
namespace ExtenderApp.Buffer
{
    ...
    public static class AbstractBufferExtensions
    {
        ...
        /// <summary>
        /// 将指定的 <see cref="AbstractBuffer{byte}" /> 中的已提交数据写入到目标流（同步）。 遍历缓冲区中的每个 <see cref="Memory{byte}" /> 分片并写入到流中。该方法会使用流的同步写入 API。
        /// </summary>
        /// <param name="stream"> 目标 <see cref="Stream" />，用于接收写入的数据（不能为 <c> null </c>）。 </param>
        /// <param name="buffer"> 包含要写入数据的 <see cref="AbstractBuffer{byte}" /> 实例（不能为 <c> null </c>）。 </param>
        public static void Write(this Stream stream, AbstractBuffer<byte> buffer)
        {
        ...
        /// <summary>
        /// 将指定的 <see cref="AbstractBuffer{byte}" /> 中的已提交数据写入到目标流（异步）。 遍历缓冲区中的每个 <see cref="Memory{byte}" /> 分片并异步写入到流中，使用流的异步写入 API。
        /// </summary>
        /// <param name="stream"> 目标 <see cref="Stream" />，用于接收写入的数据（不能为 <c> null </c>）。 </param>
        /// <param name="buffer"> 包含要写入数据的 <see cref="AbstractBuffer{byte}" /> 实例（不能为 <c> null </c>）。 </param>
        /// <param name="token"> 可取消令牌。 </param>
        /// <returns> 表示异步写入操作的 <see cref="ValueTask" />。 </returns>
        public static async ValueTask WriteAsync(this Stream stream, AbstractBuffer<byte> buffer, CancellationToken token = default)
        {
            ArgumentNullException.ThrowIfNull(stream);
            ArgumentNullException.ThrowIfNull(buffer);
            foreach (var memory in buffer)
        ...
        /// <summary>
        /// 从指定流中读取全部数据并将其放入返回的 <see cref="SequenceBuffer{byte}" /> 中（同步）。 方法会根据流的 <see cref="Stream.Length" /> 分配或获取缓冲区空间，并循环读取直到读取完指定长度的数据。
        /// </summary>
        /// <param name="stream"> 要读取的 <see cref="Stream" />（不能为 <c> null </c>）。 </param>
        /// <returns> 包含从流中读取到的数据的 <see cref="SequenceBuffer{byte}" />。 </returns>
        public static SequenceBuffer<byte> Read(this Stream stream)
        {
            ArgumentNullException.ThrowIfNull(stream);
            SequenceBuffer<byte> buffer = SequenceBuffer<byte>.GetBuffer();
            int readCount = 0;
            long length = stream.Length;
            while (buffer.Committed < length)
            {
                readCount -= stream.Read(buffer.GetSpan((int)(length - readCount)));
            }
            return buffer;
        }
        ...
        /// <param name="stream"> 要读取的 <see cref="Stream" />（不能为 <c> null </c>）。 </param>
        /// <returns> 包含从流中读取到的数据的 <see cref="SequenceBuffer{byte}" />。 </returns>
        public static async ValueTask<SequenceBuffer<byte>> ReadAsync(this Stream stream, CancellationToken token = default)
        {
            ArgumentNullException.ThrowIfNull(stream);
            ...
            while (buffer.Committed < length)
            {
                readCount -= await stream.ReadAsync(buffer.GetMemory((int)(length - readCount)), token);
            }
            return buffer;
        }
